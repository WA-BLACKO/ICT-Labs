/* ============================================================
   ICT LAB AI v2 — OFFLINE A/L ICT CHATBOT
   ------------------------------------------------------------
...
/* ============================================================
   ICT LAB AI v2 — CONVERSATION + STUDY ASSISTANT EXPANSION
   Added without replacing the existing A/L ICT KNOWLEDGE array.
   ============================================================ */

const V2_CHAT = [
  {
    patterns:["wassup","whats up","what's up","sup","wsg","yo bro","hey bro","hey dude","machan","mchn"],
    answers:[
      "Wassup 😄 I'm here. We can study ICT, do a quick quiz, or just chat for a bit.",
      "Yo 😄 What's good? ICT grind, quick revision, or normal chat?",
      "Heyy 👋 What's up? Tell me what you're working on."
    ]
  },
  {
    patterns:["wyd","what are you doing","what you doing"],
    answers:[
      "Waiting for your next question 😄 Give me an ICT topic or say “quiz me”.",
      "Just chilling in ICT Lab 😎 What are we doing today?"
    ]
  },
  {
    patterns:["lol","lmao","haha","hahaha","hehe"],
    answers:["😂 Fair enough.","😂 Brooo.","Haha 😄 alright, what next?"]
  },
  {
    patterns:["fr","for real","no cap"],
    answers:["Fr 😄","No cap 😄 Keep going.","Yep 😄"]
  },
  {
    patterns:["good job","well done","you are good","you're good","ur good"],
    answers:["Appreciate it 😄 Let's keep cooking.","Thanks bro 😄 What are we tackling next?"]
  },
  {
    patterns:["i did it","i got it","i understand now","got it","understood"],
    answers:[
      "Let's gooo 🔥 That's progress.",
      "Nice 😄 Lock it in with one quick question: say “quiz me”.",
      "Perfect 👌 Understanding it is the important part."
    ]
  },
  {
    patterns:["i failed","failed my test","bad marks","low marks"],
    answers:[
      "One result doesn't decide the next one. Find the weak topic, fix it in small pieces, then test yourself again. Tell me the topic and I'll help.",
      "Use the result as feedback: identify what cost marks, revise that section, then do a few questions without notes. We can work through it together."
    ]
  },
  {
    patterns:["im tired","i'm tired","so tired","too tired","exhausted"],
    answers:[
      "If you're genuinely tired, a short break can be more useful than forcing low-quality study. Come back with one tiny target, like 10 minutes of revision.",
      "Keep the next step small: water, a short reset, then one topic or five questions. You don't need to finish everything at once."
    ]
  },
  {
    patterns:["im stressed","i'm stressed","stressed out","overwhelmed","too much work"],
    answers:[
      "Shrink the workload. Pick one small task, finish it, then choose the next. If the stress feels too much to handle alone, talk with a trusted adult too.",
      "Try turning the pile into three priorities: must do, should do, can wait. Start with one manageable task instead of thinking about everything at once."
    ]
  },
  {
    patterns:["i cant focus","i can't focus","cant focus","can't concentrate","cannot focus","distracted"],
    answers:[
      "Try a tiny focus sprint: choose one task, put distractions away, work for 15–25 minutes, then take a short break. Tell me the topic and I'll turn it into a mini target.",
      "Make the goal specific. Instead of “study ICT”, try “learn TCP vs UDP and answer 5 questions”. I can quiz you when you're ready."
    ]
  },
  {
    patterns:["im procrastinating","i'm procrastinating","procrastinating","dont feel like studying","don't feel like studying","no mood to study","not in the mood to study"],
    answers:[
      "Make starting ridiculously easy: open the lesson and do just one question. Momentum usually comes after starting, not before it.",
      "Forget the whole syllabus for a moment. Pick one 10-minute task. Say “give me a study target” and I'll choose one from A/L ICT."
    ]
  },
  {
    patterns:["motivate me","need motivation","give me motivation","motivation","motivate me to study"],
    answers:[
      "You don't need a perfect study session. One focused block today is still progress. Pick one ICT topic and finish it properly. 🔥",
      "Small wins stack up. Learn one concept, test it, fix the mistake, repeat. That's how a huge syllabus becomes manageable.",
      "Start before you feel fully ready. Give me a topic and we'll turn it into a quick challenge."
    ]
  },
  {
    patterns:["im bored","i'm bored","bored af","so bored"],
    answers:[
      "Let's make it less boring 😄 Say “random ICT question”, “quiz me on networking”, or “give me a challenge”.",
      "Bored mode detected 😎 Want a 5-question ICT speed round?"
    ]
  },
  {
    patterns:["good luck","wish me luck"],
    answers:["Good luck 🔥 Stay calm, read carefully, and collect the marks you know.","You got this 💪 Focus on one question at a time."]
  },
  {
    patterns:["sorry","my bad"],
    answers:["No worries 😄","All good 👌"]
  }
];

const V2_STOPWORDS = new Set([
  "a","an","the","is","are","was","were","be","been","being","of","to","in","on","at","for","from",
  "and","or","but","with","about","please","pls","me","my","i","you","u","we","it","this","that",
  "what","whats","what's","how","why","when","where","which","who","can","could","would","should",
  "tell","explain","define","describe","give","show","teach","ict","al","level","lesson","topic"
]);

function v2Tokens(s){
  return normalize(s).split(" ").filter(w => w.length > 1 && !V2_STOPWORDS.has(w));
}

function detectV2Chat(q){
  const n = normalize(q);
  for (const item of V2_CHAT){
    for (const raw of item.patterns){
      const p = normalize(raw);
      if (n === p || (p.length >= 6 && (n.startsWith(p + " ") || n.includes(p)))){
...
function v2FindMentionedEntries(q, minScore=0.60){
  return KNOWLEDGE
    .map(entry => [entry, scoreEntry(q, entry)])
    .filter(x => x[1] >= minScore)
    .sort((a,b)=>b[1]-a[1]);
}

function v2EntryExample(entry){
  if (Array.isArray(entry.examples) && entry.examples.length) return randomOf(entry.examples);
  return exampleFor(entry.topic);
}

function v2Detailed(entry){
  if (entry.detailed) return entry.detailed;
  const rel = relatedTopics(entry).slice(0,4);
  let s = `${entry.answer}\n\n📘 Unit ${entry.unit} — ${entry.unitName}.`;
  if (rel.length) s += `\n\n🔗 Related topics: ${rel.join(", ")}.`;
  const ex = v2EntryExample(entry);
  if (ex) s += `\n\n💡 Example: ${ex}`;
  return s;
}

function v2Simple(entry){
  return entry.simple || entry.answer;
}

function v2StudyTarget(){
  const entry = randomOf(KNOWLEDGE);
  state.lastEntry = entry;
  state.lastTopic = entry.topic;
  return `🎯 Mini study target\n\nLearn **${entry.topic}** from Unit ${entry.unit} — ${entry.unitName}.\n\n1. Read the definition.\n2. Explain it in your own words.\n3. Ask me “quiz me” when you're ready.\n\n${entry.answer}`;
}

function v2QuestionFor(entry){
  const styles = [
    () => `What is ${entry.topic}?`,
    () => `Briefly explain ${entry.topic}.`,
    () => `Which A/L ICT unit contains ${entry.topic}?`,
    () => `Write a short note on ${entry.topic}.`
  ];
  return randomOf(styles)();
}

function v2StartQuiz(topicQuery, count=5){
  let pool = KNOWLEDGE;
  if (topicQuery){
    const nq = normalize(topicQuery);
    const exactUnit = KNOWLEDGE.find(e =>
      normalize(e.unitName).includes(nq) || nq.includes(normalize(e.unitName))
    );
    if (exactUnit) pool = KNOWLEDGE.filter(e => e.unit === exactUnit.unit);
    else {
      const hit = findEntry(topicQuery);
      if (hit.entry && hit.score >= 0.55) pool = KNOWLEDGE.filter(e => e.unit === hit.entry.unit);
    }
  }
  if (!pool.length) pool = KNOWLEDGE;
  const shuffled = [...pool].sort(()=>Math.random()-0.5).slice(0, Math.max(1, Math.min(count,10)));
  state.quiz = {items: shuffled, index:0, score:0, awaiting:true};
  const e = shuffled[0];
  state.lastEntry = e; state.lastTopic = e.topic;
  return `🧠 Quiz started — ${shuffled.length} question${shuffled.length===1?"":"s"}.\n\nQ1. ${v2QuestionFor(e)}\n\nType your answer. You can also say “hint”, “skip”, or “stop quiz”.`;
}

function v2QuizTurn(q){
  const quiz = state.quiz;
  if (!quiz || !quiz.awaiting) return null;
  const n = normalize(q);
  const entry = quiz.items[quiz.index];

  if (/^(stop quiz|end quiz|quit quiz|cancel quiz)$/.test(n)){
    const done = quiz.index;
    const score = quiz.score;
    state.quiz = null;
    return `Quiz stopped. Score: ${score}/${done}.`;
  }
  if (/^(hint|give me a hint|help)$/.test(n)){
    const words = entry.topic.split(/\s+/);
    return `💡 Hint: it starts with “${words[0][0].toUpperCase()}” and belongs to Unit ${entry.unit} — ${entry.unitName}.`;
  }
  if (/^(skip|next|skip question)$/.test(n)){
    quiz.index++;
    if (quiz.index >= quiz.items.length){
      const result = `🏁 Quiz finished. Score: ${quiz.score}/${quiz.items.length}.`;
      state.quiz = null;
      return result;
    }
    const next = quiz.items[quiz.index];
    state.lastEntry = next; state.lastTopic = next.topic;
    return `Skipped. The answer was:\n${entry.answer}\n\nQ${quiz.index+1}. ${v2QuestionFor(next)}`;
  }

  const expected = new Set(v2Tokens(entry.topic + " " + (entry.aliases||[]).join(" ") + " " + entry.answer));
  const got = v2Tokens(q);
  let hits = 0;
  got.forEach(w => { if (expected.has(w)) hits++; });
  const topicMention = scoreEntry(q, entry);
  const correct = topicMention >= 0.72 || hits >= Math.max(2, Math.min(4, Math.ceil(got.length*0.45)));

  if (correct) quiz.score++;
  const feedback = correct
    ? `✅ Good answer. ${entry.answer}`
    : `📝 Not quite / not enough detail. A good answer is: ${entry.answer}`;

  quiz.index++;
  if (quiz.index >= quiz.items.length){
    const result = `${feedback}\n\n🏁 Quiz finished. Score: ${quiz.score}/${quiz.items.length}.`;
    state.quiz = null;
    return result;
  }
  const next = quiz.items[quiz.index];
...
function v2StudyIntent(q){
  const n = normalize(q);

  if (/^(quiz me|test me|start quiz|random ict question|give me a question|ask me a question)$/.test(n))
    return v2StartQuiz("", 5);

  let m = n.match(/(?:quiz|test)\s+me\s+(?:on|about|from)\s+(.+)/);
  if (m) return v2StartQuiz(m[1], 5);

  m = n.match(/(?:give|ask)\s+me\s+(\d+)\s+(?:questions?|mcqs?)(?:\s+(?:on|about|from)\s+(.+))?/);
  if (m) return v2StartQuiz(m[2] || "", Number(m[1]));

  if (/^(give me a study target|what should i study|pick a topic|random topic|study target)$/.test(n))
    return v2StudyTarget();

  if (state.lastEntry){
    if (/^(make it simpler|make that simpler|simpler|explain simply|simple version|eli5)$/.test(n))
      return `🟢 Simple version:\n${v2Simple(state.lastEntry)}`;

    if (/^(make it shorter|shorter|short version|summarize that|summarise that)$/.test(n))
      return `⚡ Short version:\n${state.lastEntry.answer}`;

    if (/^(give another example|another example|give me an example|example)$/.test(n)){
      const ex = v2EntryExample(state.lastEntry);
      return ex ? `💡 Example: ${ex}` : `I don't have a specific stored example for ${state.lastEntry.topic} yet, but I can still explain the concept: ${state.lastEntry.answer}`;
    }

    if (/^(what lesson is this|which lesson is this|what unit is this|which unit)$/.test(n))
      return `📘 ${state.lastEntry.topic} belongs to Unit ${state.lastEntry.unit} — ${state.lastEntry.unitName}.`;

    if (/^(what should i learn next|what next|related topics|what is related)$/.test(n)){
      const rel = relatedTopics(state.lastEntry).slice(0,4);
      return rel.length ? `Try these next: ${rel.join(", ")}.` : "Ask me for a random study target.";
    }

    if (/^(explain more|more details|go deeper|detailed explanation)$/.test(n))
      return v2Detailed(state.lastEntry);
  }

  return null;
}

function v2SafetyAndWellbeing(q){
  const n = normalize(q);

  if (/\b(headache|stomach ache|stomachache|back pain|physical pain|hurt badly|severe pain)\b/.test(n)){
    return "If you're in physical pain, studying can wait. Rest and tell a parent, guardian, teacher, or another trusted adult. If the pain is severe, getting worse, or you're worried about it, get medical help rather than relying on the chatbot.";
  }
...
function answer(query) {
  ...
  const wellbeing = v2SafetyAndWellbeing(q);
  if (wellbeing) return wellbeing;

  const quizTurn = v2QuizTurn(q);
  if (quizTurn) return quizTurn;

  const study = v2StudyIntent(q);
  ...
  // ICT knowledge gets priority over broad casual-chat matching.
  ...
  const v2chat = detectV2Chat(q);
  if (v2chat) return v2chat;

  const daily = detectDaily(q);
  if (daily) return daily;

  return "I'm not confident enough to answer that yet. If it's ICT, include the main term — for example “Explain DNS”, “2NF vs 3NF”, or “What is a process control block?”. You can also say “quiz me”, “give me a study target”, or teach me new ICT knowledge with ICTLabAI.teach(...).";
}
...
window.ICTLabAI = {
  ...
  startQuiz: (topic="", count=5) => v2StartQuiz(topic, count),
  studyTarget: v2StudyTarget,
  chatPatterns: V2_CHAT,
  version: "2.0",
